...
export default function AboutPage() {
  return (
    <main className="max-w-5xl mx-auto px-6 py-12 space-y-16">
      ...
      <section className="grid md:grid-cols-4 gap-6 text-center">
        {profile.stats?.map((stat) => (
          <div key={stat.label} className="p-4 rounded-xl border shadow-sm bg-neutral-100 dark:bg-neutral-800">
            <h3 className="text-xl font-semibold text-neutral-900 dark:text-neutral-100">{stat.value}</h3>
            <p className="text-neutral-900 dark:text-neutral-100">{stat.label}</p>
          </div>
        ))}
      </section>
      ...
      {/* Mission */}
      <section>
        <h2 className="text-2xl font-bold mb-4 text-neutral-900 dark:text-neutral-100">Philosophy / Mission</h2>
        <p className="leading-relaxed text-neutral-900 dark:text-neutral-100">{profile.mission}</p>
      </section>

      {/* Languages */}
      <section>
        <h2 className="text-2xl font-bold mb-4 text-neutral-900 dark:text-neutral-100">Languages</h2>
      ...
      {/* Skills */}
      <section>
        <h2 className="text-2xl font-bold mb-4 text-neutral-900 dark:text-neutral-100">Skills</h2>
        <div className="space-y-6">
          {Object.entries(profile.skills).map(([category, skills]) => (
            <div key={category}>
              <h3 className="text-lg font-semibold mb-2 capitalize text-neutral-900 dark:text-neutral-100">{category}</h3>
              <div className="flex gap-3 flex-wrap">
                {skills.map((skill) => (
                  <span
                    key={skill}
